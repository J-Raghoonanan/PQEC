...
#     iid_needs_individual_twirling = False
    
#     if spec.noise.mode == NoiseMode.iid_p:
#         twirling_active = spec._should_apply_twirling()
        
#         if twirling_active:
#             # CANNOT cache when twirling - each copy needs different random Clifford
#             logger.info("iid_p mode with twirling: will generate each copy independently")
#             iid_needs_individual_twirling = True
#         else:
#             # Can cache when no twirling
...
#         logger.info(f"  Bloch vector magnitude: |r⃗|={r_init:.6f} (theory: 1-δ={1-spec.noise.p:.6f})")
...
#         elif iid_needs_individual_twirling:
#             # Generate each copy with different twirling seed
#             logger.debug(f"Copy {i}: Generating iid_p copy with individual twirling")
#             twirl_spec = spec.twirling
...
#     steps_path = out_dir / "steps_circuit_dephasing_M1.csv"
#     finals_path = out_dir / "finals_circuit_dephasing_M1.csv"
...
def run_streaming(spec: RunSpec) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _get_new_copy_dm(i: int) -> DensityMatrix:
        ...
        if iid_cached_dm is not None:
            ...
            logger.debug(f"Copy {i}: Using cached iid_p density matrix")
            return iid_cached_dm
        elif spec.noise.mode == NoiseMode.iid_p:
