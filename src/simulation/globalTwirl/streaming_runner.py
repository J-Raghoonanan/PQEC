...
1) REGULAR streaming mode:
   ...
   - WARNING: If Clifford twirling is enabled here and you generate different twirls per
...
2) ITERATIVE mode (Global Twirl version):
   ...
   - For each iteration t:
       (a) Apply noise ONCE to the current state to produce a single noisy density matrix.
           ...
           The Clifford C_t cycles deterministically over {I, H, HS} across iterations.
...
- cheap global/parallel twirl: 3 frames total, not 3^M.
...
# -----------------------------
# Global deterministic Clifford cycle (cheap "parallel" twirl)
# -----------------------------
...
def _apply_global_frame_then_noise(
    ...
    *,
    ...
    Otherwise:
      apply raw noise once.

    NOTE on convention:
      This implements C† Z C in the lab frame, since we do:
        rho -> C rho C†, apply Z, then undo: C† (...) C.
    """
...
# -----------------------------
# Iterative Purification Runner (Global Twirl)
# -----------------------------

def run_iterative_purification(spec: RunSpec) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Iterative purification with cheap global/parallel twirl (GlobalTwirl variant).
    ...
    # number of iterations: consistent with your current convention
    num_iterations = int(np.log2(spec.N)) if spec.N > 1 else 1

    logger.info(f"Starting ITERATIVE purification (GlobalTwirl): {spec.synthesize_run_id()}")
    ...
    if twirling_active and spec.noise.noise_type == NoiseType.dephase_z:
        logger.info("  Global Clifford cycling ENABLED for dephase_z: C_t in {I, H, HS} (deterministic by iteration)")
    elif twirling_active:
        logger.warning("  Twirling enabled by config, but GlobalTwirl iterative runner only applies global cycling for dephase_z.")
        logger.info("  Other noise types will run without twirling in iterative mode.")
    else:
        logger.info("  Twirling disabled in iterative mode")

    # Backend placeholder (iterative uses density ops; backend retained for compatibility)
    ...
    # Baseline metrics: after ONE noise application (iteration index 0)
    ...
    for iter_idx in range(num_iterations):
        ...
        # 1) Apply noise ONCE per iteration (cheap global cycling for dephase_z)
...
def run_regular_streaming(spec: RunSpec) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    WARNING about twirling:
      ...
      merges will generally not be on identical inputs. Kept as-is.
    """
    spec.validate()
    out_dir: Path = spec.out_dir
    out_dir.mkdir(parents=True, exist_ok=True)
